# anatomy.md

> Auto-maintained by OpenWolf. Last scanned: 2026-05-08T06:09:02.787Z
> Files: 42 tracked | Anatomy hits: 0 | Misses: 0

## ./

- `.DS_Store` (~2732 tok)
- `.gitignore` — Git ignore rules (~19 tok)
- `CLAUDE.md` — OpenWolf (~57 tok)
- `docker-compose.yml` — Docker Compose services (~62 tok)
- `Dockerfile` — Docker container definition (~88 tok)
- `manifest.json` (~188 tok)
- `package-lock.json` — npm lock file (~628 tok)
- `package.json` — Node.js package manifest (~65 tok)
- `playwright.config.js` (~90 tok)
- `README.md` — Project documentation (~1350 tok)

## .claude/

- `settings.json` (~441 tok)
- `settings.local.json` (~45 tok)

## .claude/rules/

- `openwolf.md` (~313 tok)

## _locales/en/

- `messages.json` (~399 tok)

## _locales/zh_CN/

- `messages.json` (~327 tok)

## e2e/

- `.DS_Store` (~2186 tok)
- `extension.js` — ", route => { (~456 tok)
- `mock-rates.js` — Mock exchange rates — all conversions go through USD (~65 tok)
- `server.js` — Declares __dirname (~312 tok)

## e2e/fixtures/

- `ambiguous.html` — Ambiguous $ (~55 tok)
- `backpack.html` — Backpack-style Table (~72 tok)
- `basic.html` — Currio Basic Tests (~105 tok)
- `codes.html` — Currency Code Formats (~109 tok)
- `dynamic.html` — Dynamic Content (~139 tok)
- `excluded.html` — Excluded Content (~131 tok)
- `fractional.html` — Fractional Amounts (~93 tok)
- `spa-update.html` — SPA Text Update (~149 tok)
- `split.html` — Split Currency Symbols (~129 tok)
- `style-preview.html` — Currio 内嵌样式预览 (~2415 tok)
- `suffixed.html` — Suffixed Amounts (~103 tok)
- `taobao-price.html` — Taobao-style Split Prices (~160 tok)

## e2e/specs/

- `conversion.spec.js` — getConversionTexts: countConversions, waitForConversion (~2390 tok)

## icons/

- `.DS_Store` (~1640 tok)

## scripts/

- `pack.sh` (~271 tok)

## src/

- `background.js` — Background service worker — rates, caching, messaging (~1460 tok)
- `content.css` — Styles: 3 rules (~416 tok)
- `content.js` — Content script — DOM scanning, currency recognition, inline rendering (~4171 tok)
- `popup.css` — Styles: 21 rules (~750 tok)
- `popup.html` — Currio (~299 tok)
- `popup.js` — Popup settings UI — changes take effect immediately (~1231 tok)
- `utils.js` — Shared currency utilities — loaded as plain script for content & background (~940 tok)

## test-results/

- `.last-run.json` (~13 tok)
